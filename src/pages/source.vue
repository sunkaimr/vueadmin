<template>
  <imp-panel>
    ...
    <div slot="body" style="min-height: 400px;">
      <el-dialog title="添加源" :visible.sync="dialogAddFormVisible" :close-on-click-modal="false" style="width: 100%;">
        <el-form size="mini" :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="集群ID" prop="cluster_id" label-width="80px">
            <el-select v-model="form.cluster_id" @change="clusterChange" filterable placeholder="请选择集群"
                       style="width: 100%;">
              <el-option
                v-for="item in clusterList" :key="item.cluster_id" :label="item.cluster_id + ' | ' + item.cluster_name"
                :value="item.cluster_id">
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="源库名" prop="database_name" label-width="80px">
            <el-select v-model="form.database_name" @change="databaseChange" filterable placeholder="请选择库名"
                       style="width: 100%;">
              <el-option
          ...
          <el-form-item label="源表名" label-width="80px">
            <div style="display: flex; align-items: center;">
              <el-input v-model="tableNameFilter" @blur="handleTableNameFilter" @clear="handleTableNameFilter"
                        style="flex: 1;" clearable placeholder="使用正则表达式筛选所需表名"/>
            </div>
            <div style="display: flex; margin-top: 10px; align-items: center;">
              <el-select v-model="form.tables_name" @visible-change="tablesNameChanged" multiple collapse-tags clearable
      ...
      <el-dialog title="修改源端信息" :visible.sync="dialogEditFormVisible" :close-on-click-modal="false"
                 style="width: 100%;">
        <el-form size="mini" :model="form" :rules="rules" ref="form">
          <el-form-item label="源端名称" prop="name" label-width="80px">
            <el-input v-model="form.name" autocomplete="off" clearable/>
